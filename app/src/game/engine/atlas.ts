...
export function createAtlas(): Atlas {
  ...
  /** 灰度图格 × 乘色（草顶/树叶图标用，平原群系色） */
  const tintedCache = new Map<string, HTMLCanvasElement>();
  const tintedTile = (name: string, rgb: [number, number, number]): HTMLCanvasElement => {
    const key = `${name}:${rgb.join(',')}`;
    const hit = tintedCache.get(key);
    if (hit) return hit;
    const src = tileCanvases.get(name)!;
    const c = document.createElement('canvas');
    c.width = TILE;
    c.height = TILE;
    const ctx = c.getContext('2d')!;
    ...
    for (let i = 0; i < img.data.length; i += 4) {
      img.data[i] = (img.data[i] * rgb[0]) / 255;
      img.data[i + 1] = (img.data[i + 1] * rgb[1]) / 255;
      img.data[i + 2] = (img.data[i + 2] * rgb[2]) / 255;
    }
    ctx.putImageData(img, 0, 0);
    tintedCache.set(key, c);
    return c;
  };
  const GRASS_ICON_TINT: [number, number, number] = [0x91, 0xbd, 0x59];
  const LEAF_ICON_TINT: [number, number, number] = [0x67, 0xb0, 0x45];
  const BIRCH_LEAF_ICON_TINT: [number, number, number] = [0x80, 0xa7, 0x55];
  const getIcon = (id: number): HTMLCanvasElement => {
    ...
    if (id === ITEM_ID_DIAMOND) {
      ...
      let side = tileCanvases.get(info.tex.side)!;
      if (info.tint === 'leaves')
        side = tintedTile(info.tex.side, id === 11 ? BIRCH_LEAF_ICON_TINT : LEAF_ICON_TINT);
      if (info.kind === 'cross' || info.kind === 'torch') {
        if (info.tint === 'grass') side = tintedTile(info.tex.side, GRASS_ICON_TINT);
        ctx.drawImage(side, 3, 3, 40, 40);
      } else {
        let top = tileCanvases.get(info.tex.top ?? info.tex.side)!;
        if (info.tint === 'grass' && info.tex.top) top = tintedTile(info.tex.top, GRASS_ICON_TINT);
        if (info.tint === 'leaves')
          top = tintedTile(
            info.tex.top ?? info.tex.side,
            id === 11 ? BIRCH_LEAF_ICON_TINT : LEAF_ICON_TINT
          );
        // 顶面
