 ...
 */
import type { PlayerInput } from '../engine-ids';
...
export class InputSystem {
  ...
  /**
   * @param target 引擎的 PlayerInput 对象（引擎每帧读取）；
   *   传入后本系统直接写入该对象，保证引擎读到最新输入。
   */
  constructor(canvas: HTMLCanvasElement, cb: InputCallbacks, target?: PlayerInput) {
    this.canvas = canvas;
    this.cb = cb;
    this.input = target ?? {
      forward: false,
